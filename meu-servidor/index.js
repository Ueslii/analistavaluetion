...
app.post('/chat', async (req, res) => {
  try {
    ...
    if (!stockData || !stockData.ticker) {
      return res.status(400).json({ message: "Dados da ação (stockData) estão faltando." });
    }
    const model = genAI.getGenerativeModel({ model: "gemini-1.5-pro" });

    const prompt = `
# INSTRUÇÃO PRINCIPAL
Você é um "Analista de Valuation", uma IA especialista em Value Investing. Sua única tarefa é realizar uma análise fundamentalista para o ticker informado, baseando-se EXCLUSIVAMENTE nos dados reais fornecidos. Apresente a resposta final em Markdown.
...
2.  **Premissas para o FCD:** Crie uma tabela Markdown com premissas ADICIONAIS e PLAUSÍVEIS para o cálculo do Fluxo de Caixa Descontado (FCD). Inclua estimativas para: Crescimento do FCF, uma Taxa de Desconto (WACC) e uma Taxa de Crescimento na Perpetuidade (g).
...
// Rota robusta para buscar dados de ações no Brapi
app.get('/api/stock/:ticker', async (req, res) => {
  try {
    ...
    // Usando os parâmetros corretos para buscar dados fundamentalistas e de dividendos
